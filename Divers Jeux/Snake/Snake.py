...
def play_snake():
    ...
    Controles :
        ...
        -m
    """
    global c,x_can,y_can,speed,fen,can,fruit,snake,dir_changeable,pts,lab_pts,pts,mode,walls
...
def can_update(update_fruit=False):
    ...
    if mode['wall']:
        can.delete('wall')
        for wall in walls:
            can.create_rectangle(wall[0]*c,wall[1]*c,(wall[0]+1)*c,(wall[1]+1)*c,fill='purple',tag='wall')
...
def play():
    ...
    if 0<=next_pix[0]<x_can and 0<=next_pix[1]<y_can and not next_pix in snake[0] and not next_pix in walls:
        if next_pix!=fruit:
            snake[0].pop(0)
...
def place_wall():
    global walls
    list_empty=[pix for pix in [(a,b) for a in range(x_can) for b in range(y_can)] if not pix in snake[0] and not pix in walls and not pix==fruit]
    walls.append(choice(list_empty))
    can_update()
...
def replace_fruit():
    ...
    #liste
    list_empty=[pix for pix in [(a,b) for a in range(x_can) for b in range(y_can)] if not pix in snake[0] and not pix in walls]
