...
def play_pong_JvO():
    """
    Lance une partie de Pong contre l'ordinateur
...
def init():
    ...
    """
    global bar_p1,bar_p2,x_can,y_can,pts_p1,pts_p2,up_p2,down_p2
...
def up_p1(event):
    """
    Indique que l'ordinateur monte sa barre
    """
...
def down_p1(event):
    """
    Indique que l'ordinateur baisse sa barre
    """
    global bar_p1,bar_speed,y_can,bary
...
def up_p2_press(event):
    """
    Indique que le joueur appuie sur la flèche du haut
    """
    global up_p2
    up_p2=True
...
def down_p2_press(event):
    """
    Indique que le joueur appuie sur la flèche du bas
    """
    global down_p2
    down_p2=True
...
def up_p2_release(event):
    """
    Indique que le joueur a relaché la flèche du haut
    """
    global up_p2
    up_p2=False
...
def down_p2_release(event):
    """
    Indique que le joueur a relaché la flèche du bas
    """
    global down_p2,ltime
    down_p2=False
...
def f_play():
    ...
    global pts_p1,pts_p2,lab_pts_p2,up_p2,down_p2,ball_color
    ...
    if bar_p1[1]-bary-c_ball<=ball[1]<=bar_p1[1]+bary+c_ball and ball[0]-c_ball<=bar_p1[0]+barx:
        #Vérifie si la balle ne nouche pas la barre de gauche
        ...
        #Vérifie si la balle ne touche pas la barre de droite
    ...
    if up_p2: #Si le joueur appuie sur haut
        if bary+10<=bar_p2[1]:
            bar_p2[1]-=bar_speed #Monte la barre du joueur
            move_p2=True #Indique qu'il faut actualiser cette barre
        else:
            move_p2=False

    elif down_p2:#Si le joueur appuie sur bas
        if bar_p2[1]<=y_can-bary-10:
            bar_p2[1]+=bar_speed #Baisse la barre du joueur
            move_p2=True #Indique qu'il faut actualiser cette barre
        else:
            move_p2=False
    else:
