...
class Player:
    ...
    def __init__(self,x,y,width=1,height=2,velocity=5):
        ...
        self._gravity_multiplier=0.95 #Nombre par lequel le vecteur saut est
...
def platformer(level_n=None,demo=False,nb_players=1, IdisplayWinScreen = True):
    global x_can,y_can,c_main,can_main,fps,fen_plat,list_players,list_levels, i_level
    global level,level_name,timed_block_cooldown,list_t_blocks,img_nb,capt, frameStart
    global displayWinScreen

    frameStart = datetime.now()
    global dict_levels

    displayWinScreen = IdisplayWinScreen
    ...
    if demo:
        dict_levels={'demo':['demo1 Mouvements','demo2 Mort','demo3 Portes et Clefs',
                             'demo4 OneWayBlock+R',
                             'demo5 Modificateurs de Saut et de Vitesse',
                             'demo6 Blocs Cassables et Chronométrés',
                             'demo7 Blocs On-Off','demo8 Lasers, Miroirs et Boutons',
                             'demo9 Récepteurs Laser', 'Have you tried crouching']}
    elif level_n=='temp':    
        dict_levels={0:['temp']}
    ...
    list_t_blocks=sort_timed_blocks(level)
     ...
     #Création du joueur
    list_players=[Player(portal_co[0],portal_co[1],width=1,height=2,velocity=c_main//6) for x in range(nb_players)]
    ...
    fen_plat.bind('<x>',lambda x:dash(0,0))
...
def play():
    ...
    Boucle principale
    """
    global level,timed_block_cooldown,after_id,img_nb, frameStart
    ...
    # print(1/(datetime.now()-frameStart).total_seconds())
...
def next_level():
    ...
    if i_level<=len(list_levels)-1:
        ...
        if len(list_levels)==1 or not displayWinScreen:
            fen_plat.destroy()
        else :
...
if __name__ == "__main__":
    platformer(demo=False,nb_players=1,  IdisplayWinScreen = False)
    platformer(demo=True,nb_players=1,  IdisplayWinScreen = False)
    #platformer(demo=False,nb_players=1, IdisplayWinScreen = True)
